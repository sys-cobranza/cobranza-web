...
@Component({
  selector: 'cobro-persona-list',
  templateUrl: './persona-list.component.html',
  styleUrls: ['./persona-list.component.scss']
})
export class PersonaListComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit(): void {
    ...
    this.data.personas().getAll().subscribe(d => {
      if (d.success) {
  ...
  matchesFilter(item: any, filter: Filter): boolean {
    ...
    if (filter.field.id === 'numeroDocumento') {
      match = item.numeroDocumento.match(re) !== null;
    } else if (filter.field.id === 'nombre') {
      match = item.nombre.match(re) !== null;
    } else if (filter.field.id === 'apellidos') {
      match = item.apellidos === filter.value;
    } else if (filter.field.id === 'direccion') {
      match = item.direccion === filter.value;
    } else if (filter.field.id === 'numeroCelular') {
      match = item.numeroCelular === filter.value;
    }
    return match;
  }
  ...
  compare(item1: any, item2: any): number {
    ...
    if (this.currentSortField.id === 'numeroDocumento') {
      compValue = item1.numeroDocumento.localeCompare(item2.numeroDocumento);
    } else if (this.currentSortField.id === 'nombre') {
      compValue = item1.nombre.localeCompare(item2.nombre);
    } else if (this.currentSortField.id === 'apellidos') {
      compValue = item1.apellidos.localeCompare(item2.apellidos);
    } else if (this.currentSortField.id === 'direccion') {
      compValue = item1.direccion.localeCompare(item2.direccion);
    } else if (this.currentSortField.id === 'numeroCelular') {
      compValue = item1.numeroCelular.localeCompare(item2.numeroCelular);
    }
